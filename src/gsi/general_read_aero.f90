...
end module aeroreadmod

subroutine general_read_nemsaero(grd,sp_a,filename,gfschem_bundle, &
...
!$$$
    ...
    use aeroreadmod, only: aerosol_reload
    use gfsreadmod, only: general_fill_ns
...
!
subroutine general_read_m2aero(grd,sp_a,filename,gfschem_bundle, &
       naero,aeroname,init_head,iret_read)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    general_read_nemsaero   adaptation of general_read_gfsatm
...
!   Declare local variables
    character(len=120) :: my_name = 'general_read_m2aero'
    character(len=1)   :: null = ' '
    character(len=20),dimension(npe) :: ch_aero
    integer(i_kind):: iret,nlatm2,nlevs,icm,nord_int
    integer(i_kind):: i,j,k,l,icount,kk,istatus,ier,kr
    integer(i_kind) :: latb, lonb, levs, nframe
    integer(i_kind) :: nfhour, nfminute, nfsecondn, nfsecondd
    integer(i_kind) :: istop = 101
    integer(i_kind),dimension(npe)::ilev,iflag,mype_use
    ...
    real(r_kind),allocatable,dimension(:,:,:) :: grid_c, grid2
    real(r_kind),allocatable,dimension(:)   :: work
    real(r_kind),allocatable,dimension(:,:,:) :: rwork3d0, rwork3d1, rwork3d2
    real(r_kind),allocatable,dimension(:) :: rlats,rlons
    real(r_single),allocatable,dimension(:) :: r4lats,r4lons
...
!   Initialize variables used below
    ...
    if(procuse)then
      ...
      aerges = open_dataset(filename, paropen=.true., mpicomm=mpi_comm_read)

      ! get dimension sizes
      ncdim = get_dim(aerges, 'lon'); lonb = ncdim%len
      ncdim = get_dim(aerges, 'lat'); latb = ncdim%len
      ncdim = get_dim(aerges, 'lev'); levs = ncdim%len
      ...
      odate(3) = idate(3)  !day
      odate(4) = idate(1)  !year
      call read_vardata(aerges, 'time', fhour) ! might need to change this to attribute later
...
!
      ...
      allocate( grid(grd%nlon,nlatm2), grid_v(grd%nlon,nlatm2) )
      if(diff_res)then
         allocate( grid_b(lonb,latb),grid_c(latb,lonb,1),grid2(grd%nlat,grd%nlon,1))
      end if
      allocate(rwork3d0(lonb,latb,1),rwork3d1(lonb,latb,1),rwork3d2(lonb,latb,1) )
      allocate(rlats(latb),rlons(lonb),r4lats(lonb*latb),r4lons(lonb*latb))
      call read_vardata(aerges, 'lon', r4lons)
      call read_vardata(aerges, 'lat', r4lats)
      do j=1,latb
        rlats(j)=deg2rad*r4lats(j)
      end do
      do j=1,lonb
        rlons(j)=deg2rad*r4lons(j)
      end do
...
!   sub-domains.
    do l=1,naero
      do k=1,nlevs
         ...
         if (mype==mype_use(icount)) then
            ! variable names in MERRA2
            select case ( trim(aeroname(l)) )
             case ('sulf')
               call read_vardata(aerges,'SO4',rwork3d0,nslice=kr, slicedim=3)
             case ('bc1')
               call read_vardata(aerges,'BCPHOBIC',rwork3d0,nslice=kr, slicedim=3)
             case ('bc2')
               call read_vardata(aerges,'BCPHILIC',rwork3d0,nslice=kr, slicedim=3)
             case ('oc1')
               call read_vardata(aerges,'OCPHOBIC',rwork3d0,nslice=kr, slicedim=3)
             case ('oc2')
               call read_vardata(aerges,'OCPHILIC',rwork3d0,nslice=kr, slicedim=3)
             case ('dust1')
               call read_vardata(aerges,'DU001',rwork3d0,nslice=kr, slicedim=3)
             case ('dust2')
               call read_vardata(aerges,'DU002',rwork3d0,nslice=kr, slicedim=3)
             case ('dust3')
               call read_vardata(aerges,'DU003',rwork3d0,nslice=kr, slicedim=3)
             case ('dust4')
               call read_vardata(aerges,'DU004',rwork3d0,nslice=kr, slicedim=3)
             case ('dust5')
               call read_vardata(aerges,'DU005',rwork3d0,nslice=kr, slicedim=3)
             case ('seas1')
               call read_vardata(aerges,'SS001',rwork3d1,nslice=kr, slicedim=3)
               call read_vardata(aerges,'SS002',rwork3d2,nslice=kr, slicedim=3)
               rwork3d0=rwork3d1+rwork3d2
             case ('seas2')
               call read_vardata(aerges,'SS003',rwork3d0,nslice=kr, slicedim=3)
             case ('seas3')
               call read_vardata(aerges,'SS004',rwork3d0,nslice=kr, slicedim=3)
             case ('seas4')
               call read_vardata(aerges,'SS005',rwork3d0,nslice=kr, slicedim=3)
            end select
            ! Convert MERRA2 mixing ratio unit from kg/kg( 10^3 g/kg ) to ug/kg( 10^-6 g/kg )
            rwork3d0=rwork3d0*1.0e+9_r_kind
  
            if (iret /= 0) call error_msg(trim(my_name),trim(filename),'tmp','read',istop+7,iret)
            if(diff_res)then
               grid_b=rwork3d0(:,:,1)
               !call fill2_ns(grid_b,grid_c(:,:,1),latb+2,lonb)
               ! MERRA2 has the value at N. and S. pole
               do j=1,latb
                  do i=1,lonb
                    grid_c(j,i,1)=grid_b(i,j)
                  end do
               end do
               call g_egrid2agrid(p_high,grid_c,grid2,1,1,vector)
               do kk=1,grd%itotsub
    ...
    if(procuse)then
       if(diff_res) deallocate(grid_b,grid_c,grid2)
       call destroy_egrid2agrid(p_high)
       deallocate(rwork3d0,rwork3d1,rwork3d2)
       deallocate(grid)
...
!   ERROR detected while reading file
1000 continue
     write(6,*)'GENERAL_READ_AEROSOL:  ***ERROR*** reading ',&
         trim(filename),' mype,iret_read=',mype,iret_read,grd%nsig,nlevs
     return
...
!   End of routine.  Return
    ...
    return
end subroutine general_read_m2aero
